...
- Commands: `/brain:enola-check`, `/brain:enola-baseline`, `/brain:enola-query <term>`.
- `readEnolaConfig()` reads `enola.enabled`, `enola.target_repo`, and `enola.binary` from `brain.config.yml`.
